...
import superpoint_dino_utils as superpoint
...
class SuperPointDINO(BaseModel):
    default_conf = {
        'nms_radius': 4,
    ...
    def _init(self, conf):
        if conf['fix_sampling']:
            print('Fix Superpoint incorrect original keypoint sampling!')
            superpoint.sample_descriptors = sample_descriptors_fix_sampling
        self.net = superpoint.SuperPointDINO(conf)
